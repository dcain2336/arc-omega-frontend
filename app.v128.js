...
// --- Diagnostics (v12.7) ---
async function _fetchJsonWithTimeout(url, ms=9000) {
...
async function runDiagnostics() {
  ...
  if (!out) return;
  out.textContent = `ARC Diagnostics v12.7\n` +
    `Time: ${new Date().toISOString()}\n` +
    `User-Agent: ${navigator.userAgent}\n\n` +
    `API_BASE: ${API_BASE}\n\n` +
    `Checking /ping...\n`;

  const ping = await _fetchJsonWithTimeout(`${API_BASE}/ping`, 9000);
  out.textContent += `- /ping -> ${ping.status}\n`;

  out.textContent += `Checking /capabilities...\n`;
  const caps = await _fetchJsonWithTimeout(`${API_BASE}/capabilities`, 9000);
  out.textContent += `- /capabilities -> ${caps.status}\n`;

  out.textContent += `\nIf either is 401: secret mismatch. If 404: path mismatch. If 502: upstream down.\n`;
}
